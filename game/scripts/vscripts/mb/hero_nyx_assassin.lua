...
function modifier_imba_vendetta_damage_stacks:DeclareFunctions() return {MODIFIER_PROPERTY_INCOMING_DAMAGE_PERCENTAGE, MODIFIER_PROPERTY_TOOLTIP,MODIFIER_PROPERTY_PREATTACK_BONUS_DAMAGE} end
...
function modifier_imba_vendetta_damage_stacks:GetModifierPreAttack_BonusDamage(keys) 
	if IsServer() then
		if self:GetParent() == keys.attacker and not self:GetParent():PassivesDisabled() and not self:GetParent():IsIllusion() and not self:GetParent():HasModifier("modifier_imba_vendetta") then 
