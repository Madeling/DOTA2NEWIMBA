...
function tree_dance:OnAbilityPhaseStart()
    ...
    if caster:HasModifier("modifier_primal_spring_motion") then 
        caster:RemoveModifierByName("modifier_primal_spring_motion")
    end
    if caster:HasModifier("modifier_tree_dance_idle") then
        caster:RemoveModifierByName("modifier_tree_dance_idle")
    end
    return true
end
...
function tree_dance:OnSpellStart()
    ...
    local dis=TG_Distance(caster_pos,target_pos)+caster:GetCastRangeBonus()
    local dir=TG_Direction2(target_pos,caster_pos)
    caster:AddNewModifier(caster, self, "modifier_tree_dance_motion", {dir=dir,dis=dis,target=target:entindex()})
    local pp =
    {
        ...
        vVelocity =caster:GetForwardVector(),
        Source = caster,
...
function modifier_tree_dance_motion:UpdateVerticalMotion(t, g)
    ...
    if TG_Distance(POS1,self.POS)>= self.DIS then
        ...
        if self:GetParent():HasModifier("modifier_tree_dance_height") then
            if (self.TARGET:GetAbsOrigin().z-POS1.z)>=256  then 
                self.H=self.H+5
            else 
                self.H=self.H+4
            end 
