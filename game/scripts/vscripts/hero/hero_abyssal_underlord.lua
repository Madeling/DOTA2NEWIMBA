...
function modifier_imba_firestorm_thinker:OnCreated()
	if IsServer() then
		...
		self:StartIntervalThink(self:GetAbility():GetSpecialValueFor("wave_interval") + self:GetCaster():TG_GetTalentValue("special_bonus_imba_abyssal_underlord_2"))
	end
end
...
function modifier_imba_firestorm_thinker:OnIntervalThink()
	...
	local dmg = ability:GetSpecialValueFor("wave_damage") + caster:TG_GetTalentValue("special_bonus_imba_abyssal_underlord_1")
...
function modifier_imba_atrophy_aura_scepter_buff:GetModifierPreAttack_BonusDamage()
	...
	return (self:GetCaster():GetModifierStackCount("modifier_imba_atrophy_aura_temp", nil) + self:GetCaster():GetModifierStackCount("modifier_imba_atrophy_aura_permanent", nil)) / 2
end
...
function imba_abyssal_underlord_dark_rift:OnSpellStart()
	...
	caster:AddNewModifier(caster, self, "modifier_imba_double_gate", {duration = self:GetSpecialValueFor("teleport_delay"), target = target and target:entindex() or caster:entindex()})
end
...
function imba_abyssal_underlord_cancel_dark_rift:Set_InitialUpgrade() return {kv=1} end
