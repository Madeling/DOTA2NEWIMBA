...
function imba_chaos_knight_reality_rift:OnSpellStart()
	...
	for i=1, #ck do
		if ck[i]:GetPlayerOwnerID() == caster:GetPlayerOwnerID() and (not ck[i]:IsCourier() and not ck[i]:GetName()=="npc_dota_flying_courier" ) then
...
function modifier_imba_phantasm_buff:OnCreated()
	...
	if IsServer() and self.parent:Has_Aghanims_Shard() then
